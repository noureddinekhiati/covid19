...
public class DashboardChart extends VerticalLayout {
    public DashboardChart(List<Datum> data, ChartType type, String title, Function<Datum, Number> getConfirmed,
                          Function<Datum, Number>getDeaths, Function<Datum, Number> getRecovered) {
    ...
    private DataSeries getDataSeries(Stream<Datum> data, Function<Datum,Number> function, String name) {
        DataSeries dataSeries = new DataSeries(
                data.map(d -> new DataSeriesItem(
                        LocalDate.parse(d.getDate()).atStartOfDay().toInstant(ZoneOffset.UTC),
                        function.apply(d))).collect(Collectors.toList()));
